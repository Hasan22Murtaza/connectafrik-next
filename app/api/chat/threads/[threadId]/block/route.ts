...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const is_block = body.is_block

    if (typeof is_block !== 'boolean') {
      return errorResponse('is_block is required and must be a boolean', 400)
    }
    ...
    if (updateError) {
      return errorResponse(updateError.message || 'Failed to update block state', 400)
    }
    ...
    return errorResponse(err.message || 'Failed to block thread', 500)
  }
}
