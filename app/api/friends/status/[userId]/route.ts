...
export async function GET(
  ...
  try {
    ...
    const accepted = list.find((r: { status: string }) => r.status === 'accepted')
    if (accepted) {
      return jsonResponse({ status: 'friends', request_id: accepted.id })
    }

    const pendingSent = list.find(
      (r: { status: string; sender_id: string }) => r.status === 'pending' && r.sender_id === user.id
    )
    if (pendingSent) {
      return jsonResponse({ status: 'pending_sent', request_id: pendingSent.id })
    }

    const pendingReceived = list.find(
      (r: { status: string; receiver_id: string }) => r.status === 'pending' && r.receiver_id === user.id
    )
    if (pendingReceived) {
