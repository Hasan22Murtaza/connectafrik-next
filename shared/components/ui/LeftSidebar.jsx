      ...
      <ul className="space-y-2">
        {onlineContacts && onlineContacts.length > 0 ? (
          onlineContacts.map((user) => {
            const displayName = user?.full_name || user?.name || 'Community member';
            const avatar = user?.avatar_url || user?.avatarUrl || null;
            const status = user?.status || (user?.lastSeen ? 'away' : 'offline');
            const statusColor = getStatusColor(status);

            return (
              <li key={user.id || displayName} className="flex items-center space-x-3">
